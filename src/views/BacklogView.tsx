...
import { CalendarClock, CheckSquare2, ListFilter, Plus, UserRound } from 'lucide-react';
...
import { dueFilterMatch, formatAssigneeList, nextOrderRank, sortTasksByRank, taskDueWeekKey } from '@/utils/tasks';
...
function SortableTaskRow({
  ...
  return (
    <div
      ...
      className={cn(
        'grid grid-cols-[minmax(260px,1.5fr)_150px_170px_130px_170px_80px] gap-3 items-center p-3 border-b border-border bg-card',
...
export function BacklogView({ projectId }: BacklogViewProps) {
  ...
  const handleDescriptionChange = (event: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    updateTask.mutate({ taskId: selectedTask.id, description: event.target.value });
  };
    ...
    <div className="h-full flex">
      <div className="flex-1 p-6 space-y-4 overflow-auto">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Add tasks quickly with required start/due dates and edit fields inline without opening modals.
            </p>
          </div>
        ...
        <div className="border border-border rounded-lg overflow-hidden">
          <div className="grid grid-cols-[minmax(260px,1.5fr)_150px_170px_130px_170px_80px] gap-3 p-3 bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
            <span>Task</span>
            <span>Assignee</span>
            <span>Due date</span>
            <span>Priority</span>
      ...
      <aside className="w-[360px] border-l border-border bg-card p-4 overflow-auto">
        {!selectedTask ? (
          <div className="h-full grid place-items-center text-center text-sm text-muted-foreground">
            <div>
              <CheckSquare2 className="mx-auto mb-2 size-6" />
              <p>Select a task and open details.</p>
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Progressive details panel</p>
            </div>

            <div className="space-y-2 text-sm">
              <div className="flex items-start gap-2">
                <UserRound className="size-4 mt-0.5 text-muted-foreground" />
                <p>{formatAssigneeList(selectedTask)}</p>
              </div>
              <div className="flex items-start gap-2">
